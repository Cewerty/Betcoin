...
contract Betcoin {
    ...
    function stake(uint256 amount, uint256 time) public isNotPaused() isNotBlacklisted() returns (bool success) {
        require(balanceOf[msg.sender] >= amount, "Not enough tokens for stake");
        require(amount > 0, "You can not stake zero tokens");
        require(maximumStakingPeriod > time, "Time period bigger than limits");
        require(minimumStakingPeriod < time, "Time is lesser than limit");
    ...
    function unstake(uint index) public isNotBlacklisted() isNotPaused() {
        ...
        delete staked[msg.sender][amount];

    }

    function claimReward(uint index) public returns(bool success) {
        uint256 totalReward = calculateReward(msg.sender, index);
        require(totalReward > 0, "No reward to claim");

        address payable payable_address = payable(msg.sender);

        payable_address.transfer(totalReward);
    ...
    function getLastRewardTime(address userAddress, uint256 index) public view returns(uint256 time) {
        for (uint i; i < lastRewardTime[userAddress].length; i++) {
            if (lastRewardTime[userAddress][i].stakeIndex == index) {
                time = lastRewardTime[userAddress][i].rewardTime;
            }
        }
        return time;
    }
    ...
    function calculateReward(address user_address, uint256 index) public view returns (uint256 reward) {
        ...
        uint256 lastTime = getLastRewardTime(user_address, index);
        uint256 stakingDuration = block.timestamp - lastTime;
        reward = stakedAmount * (stakingDuration) * rewardCof * (stakedAmount / totalStaked + 1) * (((stakingDuration / 30 days) * 5 / 100) + 1);
        return reward;
    }
}
